...
def drills_and_feedback(metrics: dict[str, Any]) -> dict[str, list[str]]:
    ...
    elif ft == "slight_side_fall":
        feedback.append("Follow-through shows some sideways movement; aim to transfer momentum more directly through the crease.")
...
def analyze_video(
    ...
    follow_through_forward_by_frame: dict[int, float] = {}
    follow_through_lateral_by_frame: dict[int, float] = {}
    follow_through_ratio_by_frame: dict[int, float] = {}
    ...
    while True:
        ...
        if result.pose_landmarks:
            ...
            if raw_angle is not None and (release_frame is None or current_frame <= release_frame):
                ...
                forward_component = abs(dy)
                lateral_component = abs(dx)
                ratio = lateral_component / (forward_component + 1e-6)

                follow_through_forward_by_frame[current_frame] = float(forward_component)
                follow_through_lateral_by_frame[current_frame] = float(lateral_component)
                follow_through_ratio_by_frame[current_frame] = float(ratio)

                if ratio < 0.35:
                    ft_feedback = "Good follow-through direction"
                    ft_color = (0, 255, 0)
                elif ratio < 0.60:
                    ft_feedback = "Slight sideways follow-through"
                    ft_color = (0, 165, 255)
                else:
                    ft_feedback = "Weight going sideways after release"
                    ft_color = (0, 0, 255)

                ft_text = f"Follow-through: ratio={ratio:.2f} | {ft_feedback}"
                cv2.putText(frame, ft_text, (40, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.9, ft_color, 3, cv2.LINE_AA)
    ...
    metrics["follow_through_direction_ratio_mean"] = None
    metrics["follow_through_direction_ratio_peak"] = None
    ...
    if rel is not None:
        ...
        ft_vals = [
            follow_through_ratio_by_frame[i]
            for i in range(rel + 1, endf + 1)
            if i in follow_through_ratio_by_frame
        ]
        if ft_vals:
            metrics["follow_through_direction_ratio_mean"] = float(np.mean(ft_vals))
            metrics["follow_through_direction_ratio_peak"] = float(np.max(ft_vals))
            mean_ratio = metrics["follow_through_direction_ratio_mean"]
            if mean_ratio < 0.35:
                metrics["follow_through_label"] = "forward_transfer_good"
            elif mean_ratio < 0.60:
                metrics["follow_through_label"] = "slight_side_fall"
            else:
                metrics["follow_through_label"] = "falling_sideways"
    ...
    if ft_label is not None:
        metrics["ratings"]["follow_through"] = "good" if ft_label == "forward_transfer_good" else "moderate" if ft_label == "slight_side_fall" else "poor"
